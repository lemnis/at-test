...
xFeature("Submit").tag("html/input/submit");
...
Scenario("MUST convey its role & name", async ({ I }) => {
  ...
  expect(ax).to.have.role(["button", "Button"]).and.have.name("Submit").and.not.be
    .disabled;
  snapshot(ax as any);
}).tag("base");
...
([ASSISTIVE_TECHNOLOGY.VOICEOVER, ASSISTIVE_TECHNOLOGY.CHROMEVOX].includes(
  ...
  Scenario(
    ...
    async ({ I }) => {
      ...
      const ax = await I.grabATOutput("#test");
      expect(await I.grabFocusedElement()).to.have.name("Submit");
      snapshot(ax as any);
    }
  ).tag("base");
});
